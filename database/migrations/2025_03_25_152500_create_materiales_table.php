...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('materiales', function (Blueprint $table) {
            ...
            $table->string('nombre')->comment('El dato nombre del Materiales');
            $table->string('descripcion', 1048)->comment('El dato descripcion del Materiales');
            $table->unsignedBigInteger('unidad_medida_id')->comment('FK a la tabla unidadesmedidas');
            $table->decimal('precio_unitario',10,2)->comment('El dato precio unitario del Materiales');
            ...
            $table->foreign('unidad_medida_id', 'fk_materiales_unidad_medida_id')
                  ->references('unidad_medida_id') // PK column in 'unidadesmedidas'
                  ->on('unidadesmedidas')          // The referenced table
                  ->onDelete('restrict')         // Prevent deleting unit if materials use it (or use 'cascade', 'set null')
    ...
    public function down(): void
    {
        ...
        Schema::table('materiales', function (Blueprint $table) {
            ...
            $table->dropForeign('fk_materiales_unidad_medida_id');
        });
        // Drop the materiales table
