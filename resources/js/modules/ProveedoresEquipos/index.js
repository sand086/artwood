...
const config = {
    moduleName: 'ProveedoresEquipos',
    baseUrl: '/proveedoresequipos/',
    idField: 'proveedor_equipo_id',
    formFields: ['proveedor_id', 'equipo_id', 'descripcion', 'unidad_medida_id', 'precio_unitario', 'detalle', 'stock', 'estado'],
    moduleForm: 'proveedoresequiposForm',
    moduleTable: 'proveedoresequiposTable',
    ...
    targetTab: 'equipos', // Nombre de la pestaña a la que se redirige despues de un evento
    formIsInModal: false, // Determina si el formulario está dentro de un modal
    showFormEventName: 'show-form-equipo',  // Nombre del evento para mostrar el formulario
    hideFormEventName: 'hide-form-equipo',
    columns: [
        // { data: 'proveedor_id', name: 'proveedor_id', title: 'Proveedor' },
        { data: 'equipo_nombre', name: 'equipo_nombre', title: 'Equipo' },
    ...
    onEditSuccess: (data) => {
        ...
        // document.querySelector('#equipos-panel').__x.$data.mostrarFormProducto = true;
        // document.getElementById('proveedoresequiposTable').closest('div[x-show]').__x.$data.mostrarFormProducto = true;
        // También podrías cambiar de pestaña si aplica
        // tab = 'equipos';
        window.dispatchEvent(new CustomEvent('show-form-equipo'));
    },
};

const proveedoresequiposModule = new BaseModule(config);
proveedoresequiposModule.init();
